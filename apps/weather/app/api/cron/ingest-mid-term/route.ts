...
import { gridMatchesShard } from "@/lib/grid-shard";
...
async function runIngestMidTerm(req: NextRequest) {
	...
	const targets = grids.filter((g) => gridMatchesShard(g.nx, g.ny, shard, total));
	...
	for (const g of targets) {
		...
		const regionKey = `${land}|${temp}`;
		if (seenRegion.has(regionKey)) {
			skippedDuplicateRegion += 1;
			continue;
		}
	...
	if (runId != null) {
		...
		if (skippedNoMeta > 0) skipParts.push(`noMeta=${skippedNoMeta}`);
		if (skippedDuplicateRegion > 0) skipParts.push(`dupRegion=${skippedDuplicateRegion}`);
		const err = errors.length ? errors.slice(0, 20).join("; ") : null;
		const summary =
			[skipParts.length ? `skip:${skipParts.join(",")}` : null, err].filter(Boolean).join(" | ") ||
			null;
		await db
			.update(ingestRuns)
			.set({
