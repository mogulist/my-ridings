...
export default function RouteViewer({ routeId, mode = "db" }: RouteViewerProps) {
	...
	const [stageEndExplorerOpen, setStageEndExplorerOpen] = useState(false);
	const [stageEndExplorerRange, setStageEndExplorerRange] = useState({
		minDistanceKm: 150,
		maxDistanceKm: 250,
	});
	...
	const mapPlanToGuestPlan = useCallback((plan: any, index: number): GuestPlan => {
		...
		const scheduleMarkerMemos = normalizeScheduleMarkerMemos(plan.schedule_marker_memos);
		return {
			id: plan.id,
			name: plan.name ?? "플랜",
	...
	const persistGuestRoute = useCallback(
		(options?: { nextDbRoute?: any; targetPlanId?: string; nextPlanPois?: PlanPoiRow[] }) => {
			...
			upsertRoute({
				...
				official_distance_km: sourceRoute.official_distance_km ?? baseRoute.official_distance_km,
				official_elevation_m: sourceRoute.official_elevation_m ?? baseRoute.official_elevation_m,
				official_start_name: sourceRoute.official_start_name ?? baseRoute.official_start_name,
				official_finish_name: sourceRoute.official_finish_name ?? baseRoute.official_finish_name,
	...
	const cpMarkers = useMemo(
		() => (route ? computeCPsOnRoute(route.points_of_interest, route.track_points) : []),
		[route],
	);
	const summitMarkers = useMemo(
		() => (route ? computeSummitsOnRoute(officialSummits, route.track_points) : []),
		...
		() => ({
			...
			stageId:
				effectiveSelectedDay != null
					? (stages.find((s) => s.dayNumber === effectiveSelectedDay)?.id ?? null)
					: null,
		}),
	...
	const handleCreateOfficialSummit = useCallback(
		async (payload: { name: string; lat: number; lng: number; elevation_m: number | null }) => {
	...
	const routeSummary = useMemo(() => {
		...
		return {
			...
			rwgpsUrl: dbRoute.rwgps_url || `https://ridewithgps.com/routes/${route.id}`,
	...
	const stageEndExplorerCandidates = useMemo<StageEndCandidate[]>(() => {
		...
		const candidateDistances = [0.12, 0.5, 0.82].map(
			(position) => Math.round((minDistanceKm + width * position) / 5) * 5,
		);
		return candidateDistances.map((distanceFromStageStartKm, index) => {
			const absoluteDistanceKm = Math.min(
				totalRouteDistanceKm,
				stageEndExplorerStartKm + distanceFromStageStartKm,
			);
			return {
				id: `stage-end-preview-${index}-${absoluteDistanceKm}`,
				label: String.fromCharCode(65 + index),
	...
	const openStageEndExplorer = useCallback(() => {
		const maximum = Math.floor(unplannedDistanceKm / 5) * 5;
		const maxDistanceKm = Math.min(250, maximum);
	...
	const handleStageEndCandidateSelect = useCallback((candidate: StageEndCandidate) => {
		setSelectedStageEndCandidateId(candidate.id);
		setMapCenterOnRouteKmRequest((prev) => ({
	...
	const handleSaveOfficialSpecs = async (payload: RouteOfficialSpecsSavePayload) => {
		...
		setDbRoute((prev: DbRouteSnapshot | null) => (prev ? { ...prev, ...saved } : saved));
	};

	const activePlanName =
		dbRoute?.plans?.find((p: { id: string }) => p.id === activePlanId)?.name ?? null;

	const routeStartDate = dbRoute?.start_date ?? null;

	const activePlanStartDate =
		dbRoute?.plans?.find((p: { id: string; start_date?: string | null }) => p.id === activePlanId)
			?.start_date ?? null;
	...
	const persistStageMeta = useCallback(
		async (stageId: string, payload: { startName: string; endName: string; memo: string }) => {
			if (isGuestMode) {
	...
	const handleScheduleMarkerMemoSave = useCallback(
		async (rowKey: string, memoTrimmed: string) => {
			...
			const normalized = normalizeScheduleMarkerMemos(updated.schedule_marker_memos);
			setDbRoute((prev: { plans?: { id: string }[] } | null) => {
				if (!prev?.plans) return prev;
				const idx = prev.plans.findIndex((p) => p.id === activePlanId);
				if (idx === -1) return prev;
		...
		async (plan: { id: string; name: string; stages?: unknown[] }) => {
			...
			try {
				...
				if (isGuestMode) {
					...
					const sourcePlanRow =
						(dbRoute?.plans ?? []).find((p: { id: string }) => p.id === plan.id) ?? plan;
				...
				const clonedScheduleMemos = normalizeScheduleMarkerMemos(
					(sourcePlanRowDb as { schedule_marker_memos?: unknown }).schedule_marker_memos,
				);
		...
		<>
			{planActionInProgress !== null && (
				...
				>
					...
					<span className="text-sm font-medium text-white drop-shadow-md">{planActionMessage}</span>
				</div>
			)}
					...
					<>
						...
						{stageEndExplorerOpen ? (
							<StageEndExplorerPane
								...
								stageStartKm={stageEndExplorerStartKm}
								totalRouteDistanceKm={totalRouteDistanceKm}
								minDistanceKm={stageEndExplorerRange.minDistanceKm}
								maxDistanceKm={stageEndExplorerRange.maxDistanceKm}
							...
							<PlanStagesPane
								...
								onExploreStageEnd={unplannedDistanceKm >= 10 ? openStageEndExplorer : undefined}
						...
						<div
							className={cn(
								...
								panelStageId ? "w-80 border-r" : "w-0 border-r border-transparent",
							)}
						>
							{panelStage ? (
								<motion.div
								...
								>
									<StageDetailPanel
										stage={panelStage}
										dateLabel={stageDayLabel(panelStage.dayNumber, effectivePlanStartDate)}
			...
			<div className="flex min-h-0 flex-1 flex-col">
					...
					{!loading && (
						<KakaoMap
							...
							explorationRangeKm={
								stageEndExplorerOpen
									? {
											startKm: stageEndExplorerStartKm + stageEndExplorerRange.minDistanceKm,
											endKm: stageEndExplorerStartKm + stageEndExplorerRange.maxDistanceKm,
										}
									: null
							}
				...
				<section className="hidden h-56 shrink-0 border-t border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 lg:block">
					<ElevationProfile
						...
						summitMarkers={summitMarkers}
						labelLayout="stagger"
						hideChips={stageEndExplorerOpen}
						explorationRangeKm={
							stageEndExplorerOpen
								? {
										startKm: stageEndExplorerStartKm + stageEndExplorerRange.minDistanceKm,
			...
			{pendingDeletion && (
				<PendingDeletionDialog
					...
					onConfirm={() => {
						...
						const stageIdx = stages.findIndex((s) => s.id === pendingDeletion.stageId);
						if (stageIdx !== -1) {
							const currentStage = stages[stageIdx];
							const nextStage = stages[stageIdx + 1];
							if (nextStage) {
								confirmNextStageDeletion(currentStage.distanceKm + nextStage.distanceKm);
							}
						}
					}}
			...
			<RouteOfficialSpecsDialog
				...
				rwgpsDistanceKm={route != null ? route.distance / 1000 : dbRoute?.total_distance}
				rwgpsElevationGainM={route?.elevation_gain ?? dbRoute?.elevation_gain ?? null}
