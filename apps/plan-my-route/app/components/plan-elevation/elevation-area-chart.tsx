		...
		<ResponsiveContainer width="100%" height={chartHeightPx != null ? chartHeightPx : "100%"}>
			...
			>
					...
					{climbGradientFillStops && (
						...
						>
							{climbGradientFillStops.map((stop, i) => (
								<stop key={i} offset={stop.offset} stopColor={stop.color} stopOpacity={0.88} />
							))}
						</linearGradient>
					)}
				</defs>
				...
				<XAxis
					dataKey="distanceKm"
					type="number"
					allowDataOverflow={explorationRangeKm != null}
					domain={
						selectedDayNumber != null || climbProfile != null || explorationRangeKm != null
							? [visibleStart, visibleEnd]
							: ["dataMin", "dataMax"]
					}
					tickFormatter={(v: number) => formatDistanceAxis(Math.round(v * 10) / 10)}
					fontSize={10}
					tick={{ fill: "#9ca3af" }}
					tickLine={false}
					axisLine={false}
				...
				{hasStages &&
					stageBoundaries.map((b) => {
						const isHoveredBoundary =
							isHoveringStageEndBoundary && selectedStage != null && b.stageId === selectedStage.id;
						return (
							<ReferenceLine
						...
						<ReferenceLine
							...
							label={
								<SummitMarkerLabel
									...
									onClick={
										!chartInteractionDisabled ? () => handleSummitClick(markerKey) : undefined
									}
								/>
							}
						/>
				...
				{explorationCandidates.map((candidate) => (
					<ReferenceLine
						key={candidate.id}
						x={candidate.distanceKm}
						stroke={candidate.selected ? "#f97316" : "#71717a"}
						strokeWidth={candidate.selected ? 2 : 1.25}
						strokeDasharray={candidate.selected ? undefined : "3 3"}
						label={{
							value: candidate.label,
							position: "insideTop",
							fill: candidate.selected ? "#ea580c" : "#71717a",
							fontSize: 11,
							fontWeight: 700,
						}}
					/>
				...
				{showGradientStrip && (
					<GradientStrip segments={gradientSegments} topGap={GRADIENT_STRIP_TOP_GAP} />
				)}
			</AreaChart>
		</ResponsiveContainer>
	);
