...
export function PlanStagesPane({
	...
	const progressPercent =
		totalRouteDistanceKm > 0
			? ((totalRouteDistanceKm - unplannedDistanceKm) / totalRouteDistanceKm) * 100
			: 0;
			...
			<div className="shrink-0 border-b border-zinc-200 px-4 py-3 dark:border-zinc-700">
				...
				{stages.length > 0 && (
					<div className="mt-0.5 flex items-center justify-between text-xs font-semibold text-zinc-500 dark:text-zinc-400">
						...
						{planId && (
							<span>시작일 {planStartDate ? formatDateForDisplay(planStartDate) : "-"}</span>
						)}
					</div>
				)}
			</div>
			<div className="relative flex min-h-0 flex-1 flex-col overflow-y-auto p-4">
				...
				<div className="space-y-2">
					{stages.map((stage, idx) => {
						...
						const isHighlighted = panelStageId === stage.id || activeStageId === stage.id;
						return (
							<StageCard
								key={stage.id}
								stage={stage}
				...
				<AddStageForm
					...
					onExploreStageEnd={onExploreStageEnd}
				/>
				{stages.length > 0 && (
